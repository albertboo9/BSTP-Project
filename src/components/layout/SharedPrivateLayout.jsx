  ...
  return (
    <div className="flex min-h-screen bg-surface-50 text-gray-900 font-sans">
      ...
      >
        ...
        <div className="h-20 flex items-center justify-between px-6 border-b border-gray-100">
          <div className="flex items-center gap-3 overflow-hidden">
            <img src="/logos/logo-bstp.jpeg" alt="BSTP" className="w-10 h-10 rounded-xl object-cover flex-shrink-0 shadow-sm" />
            {sidebarOpen && (
              <div className="flex flex-col">
                <span className="font-black text-sm tracking-tight text-gray-900 uppercase">BSTP</span>
                <span className="text-[10px] text-nexus-500 font-bold uppercase tracking-widest">Plateforme Nationale</span>
              </div>
            )}
          </div>
        </div>
      ...
      >
        <header className="h-20 bg-white/90 backdrop-blur-md border-b border-gray-100 sticky top-0 z-30 flex items-center justify-between px-6 lg:px-8">
          <div className="flex items-center gap-4">
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="hidden md:flex p-2 text-gray-400 hover:text-gray-600 transition-colors" title={sidebarOpen ? "Réduire" : "Agrandir"}>
              <Menu size={20} />
            </button>
            <button onClick={() => setMobileMenuOpen(true)} className="p-2 text-gray-400 hover:text-gray-600 md:hidden">
              <Menu size={24} />
            </button>
            <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <button onClick={() => setNotificationsOpen(!notificationsOpen)} className="p-2.5 rounded-lg bg-gray-50 border border-gray-200 text-gray-500 hover:text-gray-700 relative transition-colors">
                <Bell size={18} />
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 0.5 }} exit={{ opacity: 0 }} onClick={() => setMobileMenuOpen(false)} className="fixed inset-0 z-50 bg-black/40" />
            <motion.aside initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }}
              ...
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  <img src="/logos/logo-bstp.jpeg" alt="BSTP" className="w-8 h-8 rounded-lg object-cover" />
                  <span className="font-black text-sm text-gray-900 uppercase tracking-tight">BSTP</span>
                </div>
                <button onClick={() => setMobileMenuOpen(false)} className="p-2 text-gray-400 hover:text-gray-600"><X size={20} /></button>
              </div>
                ...
                {menuItems.map((item) => {
                  ...
                  return (
                    <Link key={item.path} to={item.path} onClick={() => setMobileMenuOpen(false)}
                      className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${isActive ? "bg-nexus-50 text-nexus-700 font-semibold" : "text-gray-500 hover:text-gray-900 hover:bg-gray-50"}`}>
                      <Icon size={18} />
              ...
              <div className="mt-auto border-t border-gray-100 pt-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {ROLES.map((role) => (
                    <button key={role.id} onClick={() => { handleRoleSwitch(role.id, role.path); setMobileMenuOpen(false); }}
                      className={`px-3 py-2 rounded-lg text-left text-[11px] font-bold border transition-colors ${user?.role === role.id ? `${role.bg} border-nexus-200 ${role.color}` : "bg-white border-gray-200 text-gray-500 hover:text-gray-900"}`}>
                      {role.label}
                    </button>
                  ))}
                </div>
                <div className="flex items-center gap-3 pt-2">
                  <div className="w-8 h-8 rounded-lg bg-nexus-50 flex items-center justify-center font-bold text-xs text-nexus-600">{user?.firstName?.[0] || "U"}</div>
                  <div className="flex-1"><p className="text-xs font-bold text-gray-900 leading-tight">{user?.firstName}</p><p className="text-[10px] text-gray-400">Connecté</p></div>
                  <button onClick={handleLogout} className="p-2 text-gray-400 hover:text-danger-500"><LogOut size={16} /></button>
                </div>
              </div>
            </motion.aside>
          </>
